...
const product: ProductInfo = {
  ...
  description: [
    ...
    'Capacidad de la mochila: 20 L.',
    'Con cremallera.',
    'Es Impermeable.',
  ],
  // Asegúrate de reemplazar esto con la ruta real de tu imagen
  imageUrl: '/images/mochila-musette.jpg', 
};

// Función para generar las estrellas visuales
const getStars = (rating: number) => {
  const fullStars = Math.floor(rating);
  // Un patrón simple para generar estrellas llenas y vacías
  const stars = '★'.repeat(fullStars) + '☆'.repeat(5 - fullStars);
  return (
    <div className="flex items-center space-x-1">
        <span className="text-lg text-yellow-500">{stars}</span>
        <span className="text-sm text-gray-500 ml-2">({product.rating})</span>
        {/* Simulación de un contador de opiniones, como (386) */}
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-white p-4 lg:p-6 rounded-lg shadow-md max-w-7xl mx-auto">
      ...
      <div className="lg:w-1/2 p-4 flex justify-center items-start border-r border-gray-200">
        <img 
          src="https://www.paylesscolombia.co/50888-thickbox_default/women_s_backpack.jpg"
          alt={product.name} 
      ...
      <div className="lg:w-1/2 p-4 lg:pl-10 space-y-4">
        ...
        <div className="border-t border-gray-200 pt-4">
          ...
          {/* Simulación del descuento, como en la imagen */}
